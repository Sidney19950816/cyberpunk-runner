...
#pragma warning restore 0649
        ...
        Image[] WheelImages;
        Dictionary<DamageableObject, System.Action<float>> ActionsDict = new();
        ...
        public override bool Initialize (VehicleController vehicle)
        {
            ...
            for (var i = 0; i < Vehicle.Wheels.Length; i++)
            {
                var localPos = new Vector2(Vehicle.Wheels[i].LocalPositionOnAwake.x, Vehicle.Wheels[i].LocalPositionOnAwake.z) *  (MaxLocalDistanceBetweenWheels / maxDistance);
        ...
        {
            Color color;
            var offSpeed = 10 * Time.deltaTime;
            if (HandbrakeIcon)
            {
                color = HandbrakeIcon.color;
                if (Car.InHandBrake)
        ...
        {
            float t = 0;
            var startColor = CurrentGearText.color;
            Color currentColor;
            while (t < 1)
            {
