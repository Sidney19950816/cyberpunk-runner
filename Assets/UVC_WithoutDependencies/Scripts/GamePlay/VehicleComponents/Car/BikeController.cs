...
namespace PG
{
    public class BikeController : CarController
    {
        ...
        public static BikeController Instance
        {
            ...
            {
                if (instance == null)
                    instance = FindObjectOfType(typeof(BikeController)) as BikeController;
        ...
        private Bike _bike;
        private Vector3 retainedVelocity = Vector3.zero;
        private Vector3 restartVehicleStartPoint = Vector3.zero;

        private FightScene currentFightTrigger;
        ...
        public event Action OnCrashAction;   //Action executed in case of a crash.
...
/*                if (gForce.sqrMagnitude > Bike.MaxSqrGForceForCrash || currentVelocity.z < -Bike.MaxReverseSpeedForCrash && VehicleIsGrounded)
                ...
                var roll = Vector3.Angle(right, transform.right) * Mathf.Sign(transform.right.y);

                var targetRoll = Bike.SpeedRollAngle.Evaluate(
                    CurrentSpeed *
                    (
                        CurrentSteerAngle / (Steer.EnableSteerLimit? Steer.SteerLimitCurve.Evaluate(CurrentSpeed): 1) / Steer.MaxSteerAngle
                    )
                );
                //targetRoll = targetRoll * 2;
                ...
                if (CarControl != null)
                {
                    ...
                    if (VehicleIsGrounded)
                    {
                        ...
                        var distanceBetweenWheels = ((FrontWheel.transform.position.y - RearWheel.transform.position.y) / Bike.MaxHeightDiffWheelsForChangeCom);
                    ...
                    {
                        RB.drag = _bike.Height / 10;

                        if (_bike.Height > 1)
                            RB.AddForce(_bike.AirForce * Vector3.down, ForceMode.Acceleration);

                        //AngularVelocity.x changing on Pitch change, if !VehicleIsGrounded.
                        AdditionalPitchAngular = 0;// Bike.MaxPitchAngularInAir * CarControl.Pitch;
                    }
                }
                ...
                var targetAngularVelocity = transform.InverseTransformDirection (RB.angularVelocity);
                if(VehicleIsGrounded)
                {
                ...
                else
                {
                    ...
                    {
                        targetAngularVelocity.x = 0.2f;
                    }
                    else if(currentVelocity.z > 15)
                    {
                        targetAngularVelocity.x = 0.1f;
                    }
                    else
                    {
                        targetAngularVelocity.x = 0.4f;
                    }
                }
        ...
        public void BikeFreezeState(bool enabled)
        {
            //player.SetInFightState(enabled);
            //if (enabled)
            //{
            //    retainedVelocity = rigidBody.velocity;
            //}
            //rigidBody.constraints = enabled ? RigidbodyConstraints.FreezeAll : RigidbodyConstraints.None;
            //rigidBody.velocity = retainedVelocity;
            //if (!enabled && currentFightTrigger != null)
            //{
            //    currentFightTrigger.gameObject.GetComponent<BoxCollider>().enabled = false;
            //    currentFightTrigger = null;
            //}
        }

        private void CheckToResetVehicle()
        {
            var toOther = transform.position - _bike.ResetPosition;

            if (Vector3.Dot(toOther.normalized, transform.forward) < -0.75f)
            {
                if (restartVehicleStartPoint == Vector3.zero)
                {
                    restartVehicleStartPoint = transform.position;
                }
                else
                {
                    if (Vector3.Distance(restartVehicleStartPoint, transform.position) > 10)
                    {
                        restartVehicleStartPoint = Vector3.zero;
                        ResetVehicle();
                    }
                }
            }
            else if (Vector3.Dot(toOther.normalized, transform.forward) > 0.75f)
            {
                restartVehicleStartPoint = Vector3.zero;
            }
        }
        ...
        public override void ResetVehicle()
        {
            ...
            transform.eulerAngles = _bike.ResetAngles;
        }
