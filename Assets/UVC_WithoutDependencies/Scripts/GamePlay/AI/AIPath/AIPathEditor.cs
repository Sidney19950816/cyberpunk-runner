...
namespace PG
{
    ...
    public partial class AIPath :MonoBehaviour
    {
        ...
        private void DrawGizmos (bool selected)
        {
            ...
            if (Waypoints.Count > 1)
            {
                ...
                var prevOvertakeLeft = Waypoints[0].OvertakeZoneLeft;
                var prevOvertakeRight = Waypoints[0].OvertakeZoneRight;
                ...
                {
                    var next = GetRoutePosition (dist + EditorVisualisationSubsteps, out nextOvertakeLeft, out nextOvertakeRight, out speedLimit);
                    if (selected)
                    {
                        var dir = next - prev;
    ...
    public class WaypointDataDrawer :PropertyDrawer
    {
        public override void OnGUI (Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var floatWidth = position.width * 0.2f;
    ...
    {
        AIPath AIPath => target as AIPath;
        List<AIPath.WaypointData> Waypoints => AIPath.Waypoints;
        int HighlightedIndex { get => AIPath.HighlightedIndex;
            set => AIPath.HighlightedIndex = value;
        }
        ...
        private void OnSceneGUI ()
        {
            ...
            for (var i = 0; i < WaypointScreenPositions.Length; i++)
            {
        ...
        void UpdateList ()
        {
            ...
            for (var i = 0; i < AIPath.Waypoints.Count; i++)
            {
        ...
        void CopyWaypoints ()
        {
            var waypointsForSave = new WaypointDataDebugList();
            foreach (var w in Waypoints)
            {
        ...
        void PasteWaypoints ()
        {
            try
            {
                ...
                var waypointsForSave = JsonUtility.FromJson<WaypointDataDebugList>(EditorGUIUtility.systemCopyBuffer);
                if (waypointsForSave != null)
                {
                    for (var i = 0; i < waypointsForSave.DataList.Count; i++)
                    {
        ...
        {
            public List<WaypointDataDebug> DataList = new();
        }
