...
#pragma warning disable 0649
        ...
        public List<MeshFilter> IgnoreDeformMeshes = new();
        public List<MeshCollider> IgnoreDeformColliders = new();
        public List<Transform> IgnoreFindInChildsMeshesAndColliders = new();
...
#pragma warning restore 0649
        ...
        Vector3 CenterPoint => TR.TransformPoint (LocalCenterPoint);
        ...
        {
            if (col.gameObject.GetComponent<IgnoreDamage>() != null)
                return;
            ...
            float massFactor;
            Rigidbody rb;
            var contactsCount = Mathf.Min(col.contacts.Length, MaxContactPoints);

            var impuls = (col.impulse.magnitude * 0.001f).Clamp (0, MaxImpulseMagnitude);
            var relativeVelocityMagnitude = col.relativeVelocity.magnitude;
        ...
        {
            var damagePoint = data.DamagePoint;
            var damageForce = data.DamageForce;
            var surfaceNormal = data.SurfaceNormal;
            ...
            var clampForce = Vector3.ClampMagnitude(damageForce, MaxCollisionMagnitude);                                //Limiting force if force exceeds maximum.
            var normalizedForce = clampForce.normalized;
            var forceMagFactor = clampForce.magnitude * DamageFactor * data.MassFactor;                                   //Accept all existing factors.
            var maxDamageRadius = MaxDeformRadiusInMaxMag * (forceMagFactor / MaxCollisionMagnitude);
            var sqrMaxDamageRadius = Mathf.Pow(maxDamageRadius, 2);
            var maxDeformDist = DeformMultiplier * (forceMagFactor / MaxCollisionMagnitude);
            var sqrMaxDeformDist = Mathf.Pow (maxDeformDist, 2);                                                          //Calculation of the square of the maximum damage distance.
            var surfaceDot = Mathf.Clamp01 (Vector3.Dot (surfaceNormal, normalizedForce)) *
            ...
            var deformSurfaceDot = surfaceDot * 0.01f * DamageFactor              ;                                       //Applying all multipliers and decreasing by 100 surfaceDot for an adequate result.
            ...
            var damageBounds = new Bounds(Vector3.zero, new Vector3(maxDamageRadius, maxDamageRadius, maxDamageRadius));

            //Damage to all meshes.
            for (var i = 0; i < DamageableMeshes.Length; i++)
            {
                ...
                for (var j = 0; j < DamageableMeshes[i].Verts.Length; j++)
                {
            ...
            for (var i = 0; i < DetachableObjects.Length; i++)
            {
                ...
                for (var j = 0; j < detachableObject.DamageCheckPoints.Length; j++)
                {
