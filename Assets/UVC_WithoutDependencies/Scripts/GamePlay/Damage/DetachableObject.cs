    ...
    public class DetachableObject :MonoBehaviour
    {
        ...
        void Update ()
        {
            ...
            if (IsLoose)
            {
                ...
                {
                    for (var i = 0; i < DestroyableChilds.Length; i++)
                    {
                ...
                var needJointBreak = !Hinge || transform.parent != null && (transform.parent.TransformPoint (OnStartJointPos) - transform.TransformPoint (Hinge.anchor)).sqrMagnitude > 0.25;

                if (Hinge && chosenJoint.UseDetachSpeed)
                {
                    //Applying a spring to a detachable part
                    var speedNormalize = Mathf.InverseLerp(0, chosenJoint.maxDetachSpeed, Vehicle.CurrentSpeed);
                    var signDirection = Mathf.Sign (chosenJoint.carDirection * Vehicle.VehicleDirection);
