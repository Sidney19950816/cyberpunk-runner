...
public class Q_Vignette_Single : Q_Vignette_Base
{
    ...
    {
        StartCoroutine(LerpMainScale(0, 1.5f, 0.2f));
    }

    IEnumerator LerpMainScale(float startValue, float endValue, float lerpDuration)
    {
        float timeElapsed = 0;
        while (timeElapsed < lerpDuration)
        {
            mainScale = Mathf.Lerp(startValue, endValue, timeElapsed / lerpDuration);
