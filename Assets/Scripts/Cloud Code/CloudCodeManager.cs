...
namespace Unity.Services.CloudCode
{
    public class CloudCodeManager : MonoBehaviour
    {
        ...
        public async Task<MotorbikeBatteryEventManager.GetStatusResult> CallBatteryGetStatusEndpoint()
        {
            ThrowIfNotSignedIn();

            try
            {
                return await CloudCodeService.Instance.CallEndpointAsync<MotorbikeBatteryEventManager.GetStatusResult>(
                    "MotorbikeBattery_GetStatus",
                    new Dictionary<string, object>());
            }
            catch (CloudCodeException e)
            {
        ...
        public async Task<MotorbikeBatteryEventManager.UpdateResult> CallBatteryUpdateEndpoint(int batteryChangeAmount)
        {
            try
            {
                var updatedState = await CloudCodeService.Instance.CallEndpointAsync<MotorbikeBatteryEventManager.UpdateResult>(
                    "MotorbikeBattery_Update",
                    new Dictionary<string, object> { { "batteryChangeAmount", batteryChangeAmount } });
            ...
            catch (CloudCodeException e)
            {
                ...
                throw new CloudCodeResultUnavailableException(
                    e, $"Handled exception in {nameof(CallBatteryUpdateEndpoint)}.");
            }
        }
